...
import 'package:routelog_project/features/routes/widgets/route_polyline_map.dart';
import 'package:routelog_project/core/data/models/route_log.dart';

/// 기존: 타이틀만 받아서 목업 데이터를 보여주는 화면(호환용)
...
class _RouteDetailFromModel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final t = Theme.of(context).textTheme;
    final hasPath = model.path.length >= 2;
    ...
    return Scaffold(
      ...
      body: AppBackground(
        child: ListView(
          padding: const EdgeInsets.fromLTRB(16, 12, 16, 16),
          children: [
            // 경로가 있으면 폴리라인 지도, 없으면 기존 헤더맵
            hasPath
                ? RoutePolylineMap(
              path: model.path,
              height: 220,
              // autoFit 기본값 true (경로 전체가 화면에 보이도록)
            )
                : const RouteHeaderMap(height: 220),
            const SizedBox(height: 12),
            ...
            RouteActionBar(
              ...
              onToggleFavorite: (fav) =>
                  _snack(context, fav ? '즐겨찾기 추가' : '즐겨찾기 해제'),
              onExport: () => _snack(context, '내보내기(목업)'),
              onShare: () => _snack(context, '공유(목업)'),
              onDelete: () => _snack(context, '삭제(목업)'),
            ),
            ...
            Text('메모', style: t.titleMedium?.copyWith(fontWeight: FontWeight.w700)),
            const SizedBox(height: 8),
            Text(model.notes ?? '메모가 없습니다.', style: t.bodyMedium),
          ],
        ),
      ),
    );
